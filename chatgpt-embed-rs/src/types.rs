...
pub struct EmbeddingRequest<'a> {
    /// Specifies the input to an embedding request.
    pub input: &'a str,

    /// Selects an embedding engine.
    pub model: &'a str,
}
...
pub enum ServerResponse {
    ...
    /// Embedding completion successfully
    EmbeddingCompletion(EmbeddingCompletionResponse),
}
...
/// Contains all JSON related data for an embedding request response.
#[derive(Debug, Clone, PartialEq, PartialOrd, Deserialize)]
pub struct EmbeddingDataObject {
    /// Describes the type of embedding.
    pub object: String,
    /// Contains the index.
    pub index: u32,
...
/// Contains all JSON related data for an embedding request response.
#[derive(Debug, Clone, PartialEq, PartialOrd, Deserialize)]
pub struct EmbeddingCompletionResponse {
    ...
    /// Describes the data list.
    #[serde(rename = "data")]
    pub data_choices: Vec<EmbeddingDataObject>,
}
...
impl EmbeddingCompletionResponse {
    /// A shortcut to access the message response
    pub fn embeddings(&self) -> &Vec<f32> {
        // Unwrap is safe here, as we know that at least one message choice is provided
        &self.data_choices.first().unwrap().embedding_data
    }
}
